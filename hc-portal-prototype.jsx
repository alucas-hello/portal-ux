...
export default function PortalApp() {
  ...
  const allNavItems = [
    ...
    { id: "reports", label: "Reports", icon: Icons.Reports, roles: ["admin", "counselor"] },
  ...
  return (
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700&display=swap'); body { font-family: 'Montserrat', sans-serif; margin: 0; padding: 0; }`}</style>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-2 flex items-center justify-between" style={{ backgroundColor: "#55478f" }}>
            ...
            <div className="flex items-center gap-3">
              <input type="text" placeholder="Search for a student..." className="px-4 py-2 rounded-lg text-sm w-72" style={{ backgroundColor: "#ffffff", border: "1px solid #c0bad4", color: "#474747", outline: "none" }} />
          ...
          <div className="border-b" style={{ backgroundColor: "white", borderBottomColor: "#e5e0f0" }}>
            <div className="px-6 py-3 flex items-center justify-between">
              <h1 className="text-xl font-bold" style={{ color: "#281d51" }}>
                {currentPage === "student-detail" && selectedStudent ? `${selectedStudent.lastName}, ${selectedStudent.firstName}` : "Dashboard"}
              </h1>
              <div className="flex items-center gap-3">
      ...
      <div className="flex gap-4 mb-6 items-center">
        ...
        <div className="flex gap-2">
          {[2025, 2026, 2027].map((year) => (
            <button key={year} onClick={() => setGradYearFilter(gradYearFilter === year ? "all" : year.toString())} className="px-3 py-2 rounded-full text-sm font-medium transition-all" style={{ backgroundColor: gradYearFilter === year ? "#55478f" : "#e5e0f0", color: gradYearFilter === year ? "white" : "#474747" }}>{year}</button>
          ))}
        </div>
      </div>
        ...
        {filteredStudents.map((student) => (
          <div key={student.id} className="bg-white rounded-lg shadow-sm p-5 hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-5 gap-2 mb-2">
              <MetricChip label="Meetings" value={student.meetings} />
              <MetricChip label="Last Meeting" value={student.lastMeeting} color={getStatusColor(daysSince(student.lastMeeting))} />
              <MetricChip label="Next Contact" value={student.nextContact} />
              <MetricChip label="Next Meeting" value={student.nextMeeting || "TBD"} />
              <MetricChip label="Next Workshop" value={student.nextWorkshop === "NOT BOOKED" ? "NOT BOOKED" : student.nextWorkshop} color={student.nextWorkshop === "NOT BOOKED" ? "#ff3467" : undefined} />
            </div>
...
function MetricChip({ label, value, color }) {
  return (
    <div className="rounded-lg p-3" style={{ backgroundColor: "#f0ece8", border: "1px solid #e5e0f0" }}>
      ...
      <p className="text-sm font-bold" style={{ color: color || "#281d51" }}>{value}</p>
    </div>
  );
}
          ...
          <div key={student.id} className="bg-white rounded-lg shadow-sm p-5">
            ...
            <div className="grid grid-cols-6 gap-3">
              {essayMilestones.map((milestone, idx) => (
                <div key={idx} className="text-center"><p className="text-xs font-semibold mb-2" style={{ color: "#474747" }}>{milestone}</p><div className="w-5 h-5 rounded-full mx-auto" style={{ backgroundColor: Math.random() > 0.5 ? "#00e6c3" : "#fbbf24" }} /></div>
              ))}
            </div>
          </div>
        ))}
